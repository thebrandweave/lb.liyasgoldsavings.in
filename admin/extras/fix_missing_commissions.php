...
try {
    // 1. Fetch all active customers who have at least one verified payment
    $query = "
        SELECT DISTINCT 
            c.CustomerID, 
            c.CustomerUniqueID, 
    ...
    foreach ($verifiedCustomers as $cust) {
        ...
        if ($commissionAmount <= 0) {
            echo "⚠️ Promoter {$promoter['Name']} ($promoterUniqueID) has 0 commission configured. Skipping.\n";
            $skippedCount++;
            continue;
        }
        ...
        $alreadyCredited = $checkStmt->fetch(PDO::FETCH_ASSOC)['already_credited'];

        if ($alreadyCredited > 0) {
            echo "ℹ️ Commission already credited for customer $custName ($custUniqueID) to promoter {$promoter['Name']} ($promoterUniqueID).\n";
            $skippedCount++;
            continue;
        }

        // Proceed to credit commission
        try {
            $conn->beginTransaction();

            // 1. Update or Create Direct Promoter Wallet
            ...
            // 2. Insert Direct Promoter WalletLog
            ...
            // 3. Process Parent Promoter Commission if applicable
                ...
                if ($parentCommissionAmount > 0) {
                    ...
                    if ($parentPromoter) {
                        ...
                        if ($parentAlreadyCredited == 0) {
                            ...
                            echo "   └─ ✅ Credited Parent Commission ₹$parentCommissionAmount to Parent Promoter {$parentPromoter['Name']} ($parentPromoterID).\n";
                        }
                    }
                }
            }
    ...
    echo "\n===================================================\n";
    echo " SUMMARY: Credited: $creditedCount | Skipped/Already Credited: $skippedCount\n";
    echo "===================================================\n";
