...
if (isset($_POST['action']) && isset($_POST['promoter_id'])) {
    ...
    try {
        ...
        $stmt = $conn->prepare("SELECT p.*, pw.BalanceAmount FROM Promoters p LEFT JOIN PromoterWallet pw ON p.PromoterUniqueID = pw.PromoterUniqueID WHERE p.PromoterUniqueID = ?");
        ...
        $smsMessage = "Dear " . $promoter['Name'] . ", your wallet has been " .
            ($action === 'add' ? 'credited' : 'debited') . " with Rs " . number_format($amount, 0) .
            ". New balance: Rs " . number_format($newBalance, 0);
        if (!empty($message)) {
...
// Get promoters with wallet information directly from PromoterWallet table
$query = "SELECT p.*, 
                 COALESCE(pw.BalanceAmount, 0) as BalanceAmount, 
                    ...
                    <?php foreach ($promoters as $promoter): ?>
                        <div class="wallet-card" data-promoter-id="<?php echo $promoter['PromoterUniqueID']; ?>">
                            ...
                            <form action="" method="POST" class="wallet-form">
                                <input type="hidden" name="promoter_id" value="<?php echo $promoter['PromoterUniqueID']; ?>">
            ...
            fetch(`get_wallet_logs.php?promoter_id=${promoterId}`)
                ...
                .then(data => {
                    ...
                    container.innerHTML = data.map(log => {
                        ...
                        const isCommission = log.type === 'commission';

                        return `
                            <div class="log-entry ${isCredit ? 'credit' : 'debit'}${isCommission ? ' commission-log' : ''}">
