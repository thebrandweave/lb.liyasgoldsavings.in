...
try {
    $stmt = $conn->prepare("SELECT PromoterID, Name, PromoterUniqueID FROM mp_promoters WHERE Status = 'Active'");
    $stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (!empty($errors)) {
            ...
            $action = "Updated mp_customer details for " . $name . " (ID: " . $customer['CustomerUniqueID'] . ")";
