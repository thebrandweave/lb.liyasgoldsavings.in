...
try {
    $stmt = $conn->prepare("SELECT PromoterID, Name, PromoterUniqueID, Commission FROM mp_promoters WHERE Status = 'Active'");
    $stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (!empty($errors)) {
            ...
            $stmt = $conn->prepare("SELECT COUNT(*) FROM mp_promoters WHERE PromoterUniqueID = ?");
