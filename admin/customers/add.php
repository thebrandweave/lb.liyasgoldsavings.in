...
$stmt = $conn->prepare("SELECT PromoterID, Name, PromoterUniqueID FROM Promoters WHERE Status = 'Active' ORDER BY Name");
$stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
   ...
   try {
            ...
            // 1. Insert customer (CustomerUniqueID remains NULL for now)
            $stmt = $conn->prepare("
                INSERT INTO Customers (
                    Name, Contact, Email, PasswordHash, 
            ...
            // 2. Get the newly generated CustomerID (e.g., 1000)
            $customerId = $conn->lastInsertId();
            
            // 3. Create the unique string (e.g., "LB1000")
            $customerUniqueID = 'LB' . $customerId;

            // 4. Update the row to save the new CustomerUniqueID
