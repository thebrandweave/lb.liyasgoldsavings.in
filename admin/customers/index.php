...
// Add scheme filtering
if (!empty($schemeFilter)) {
    if ($schemeFilter === 'unsubscribed') {
        // Customers with no subscriptions AND no payments at all
        $conditions[] = "NOT EXISTS (SELECT 1 FROM Subscriptions sub_none WHERE sub_none.CustomerID = c.CustomerID)";
        $conditions[] = "NOT EXISTS (SELECT 1 FROM Payments p_any WHERE p_any.CustomerID = c.CustomerID)";
    } else {
        // Customers subscribed to the selected scheme
        $conditions[] = "EXISTS (
            SELECT 1 FROM Subscriptions sub 
            WHERE sub.CustomerID = c.CustomerID AND sub.SchemeID = :schemeId
        )";
        $params[':schemeId'] = $schemeFilter;
    }
}

// Add installment filtering (require a subscription to the installment's scheme)
if (!empty($installmentFilter)) {
    $conditions[] = "EXISTS (
        SELECT 1 
        FROM Subscriptions sub 
        JOIN Installments inst_sub ON inst_sub.SchemeID = sub.SchemeID 
        WHERE sub.CustomerID = c.CustomerID 
          AND inst_sub.InstallmentID = :installmentId
    )";
...
if (!empty($paymentStatusFilter)) {
    if ($paymentStatusFilter === 'unpaid') {
        // Show customers who are subscribed and have at least one installment (in any subscribed scheme)
        // with no Verified/Pending payment recorded
        $conditions[] = "EXISTS (
            SELECT 1
            FROM Subscriptions sub
            JOIN Installments i ON i.SchemeID = sub.SchemeID
            WHERE sub.CustomerID = c.CustomerID
              AND NOT EXISTS (
                  SELECT 1 FROM Payments p_unpaid
                  WHERE p_unpaid.CustomerID = c.CustomerID
                    AND p_unpaid.InstallmentID = i.InstallmentID
                    AND p_unpaid.Status IN ('Verified','Pending')
              )
        )";
    } else {
        $conditions[] = "EXISTS (SELECT 1 FROM Payments p_status WHERE p_status.CustomerID = c.CustomerID AND p_status.Status = :paymentStatus)";
        $params[':paymentStatus'] = ucfirst($paymentStatusFilter);
    }
}

// - If unpaid filter is selected, require any subscription to exist
if (!empty($paymentStatusFilter) && $paymentStatusFilter === 'unpaid') {
    $conditions[] = "EXISTS (SELECT 1 FROM Subscriptions sub_any WHERE sub_any.CustomerID = c.CustomerID)";
}
...
if (isset($_GET['export']) && $_GET['export'] == 'excel') {
    ...
    if (!empty($schemeFilter)) $filename .= '_scheme_' . $schemeFilter;
    if (!empty($installmentFilter)) $filename .= '_installment_' . $installmentFilter;
    if (!empty($paymentStatusFilter)) $filename .= '_payment_' . strtolower($paymentStatusFilter);
...
// Get all active schemes for filter dropdown
try {
    $schemeQuery = "SELECT DISTINCT s.SchemeID, s.SchemeName FROM Schemes s 
                   JOIN Payments p ON s.SchemeID = p.SchemeID 
...
// Get active installments for filter dropdown (filtered by selected scheme if provided)
try {
    $installmentQuery = "SELECT DISTINCT i.InstallmentID, i.InstallmentName, i.InstallmentNumber, s.SchemeName 
                        FROM Installments i 
                        JOIN Schemes s ON i.SchemeID = s.SchemeID 
                        JOIN Payments p ON i.InstallmentID = p.InstallmentID 
                        WHERE i.Status = 'Active'";

    // If a scheme is selected, restrict installments to that scheme for initial render
    if (!empty($schemeFilter)) {
        $installmentQuery .= " AND i.SchemeID = :schemeFilterForInstallments";
    }

    $installmentQuery .= " ORDER BY s.SchemeName ASC, i.InstallmentNumber ASC";

    $stmt = $conn->prepare($installmentQuery);
    if (!empty($schemeFilter)) {
        $stmt->bindValue(':schemeFilterForInstallments', $schemeFilter, PDO::PARAM_INT);
    }
    $stmt->execute();
...
<head>
    ...
    <style>
        ...
        .scheme-select, .installment-select {
            min-width: 250px;
        }
...
<body>
        ...
        <div class="page-header">
            ...
            <div class="export-btn">
                <a href="?export=excel<?php
                                        ...
                                        echo !empty($schemeFilter) ? '&scheme_id=' . urlencode($schemeFilter) : '';
                                        echo !empty($installmentFilter) ? '&installment_id=' . urlencode($installmentFilter) : '';
                                        echo !empty($paymentStatusFilter) ? '&payment_status=' . urlencode($paymentStatusFilter) : '';
            ...
            <div class="card-body">
                <form action="" method="GET" class="filter-container">
                    ...
                    <div class="filter-group">
                        ...
                        <select name="scheme_id" id="scheme_id" class="filter-select scheme-select">
                            <option value="">All Schemes</option>
                            <option value="unsubscribed" <?php echo ($schemeFilter === 'unsubscribed') ? 'selected' : ''; ?>>Unsubscribed</option>
                            <?php foreach ($schemes as $scheme): ?>
                                <option value="<?php echo htmlspecialchars($scheme['SchemeID']); ?>"
                                    <?php echo ($schemeFilter == $scheme['SchemeID']) ? 'selected' : ''; ?>>
                    ...
                    <div class="filter-group">
                        ...
                        <select name="installment_id" id="installment_id" class="filter-select installment-select">
                            <option value="">All Installments</option>
                            <?php foreach ($installments as $installment): ?>
                                <option value="<?php echo htmlspecialchars($installment['InstallmentID']); ?>"
                                    <?php echo ($installmentFilter == $installment['InstallmentID']) ? 'selected' : ''; ?>>
                    ...
                    <div class="filter-group">
                        ...
                        <select name="payment_status" id="payment_status" class="filter-select">
                            ...
                            <option value="verified" <?php echo ($paymentStatusFilter === 'verified') ? 'selected' : ''; ?>>Verified</option>
                    ...
                    <?php if (!empty($search) || !empty($status) || !empty($promoterId) || !empty($referredBy) || !empty($schemeFilter) || !empty($installmentFilter) || !empty($paymentStatusFilter)): ?>
                        ...
                        <ul class="pagination">
                            <?php if ($page > 1): ?>
                                <li>
                                    <a href="?page=1<?php echo !empty($search) ? '&search=' . urlencode($search) : '';
                                                    ...
                                                    echo !empty($schemeFilter) ? '&scheme_id=' . urlencode($schemeFilter) : '';
                                                    echo !empty($installmentFilter) ? '&installment_id=' . urlencode($installmentFilter) : '';
                                                    echo !empty($paymentStatusFilter) ? '&payment_status=' . urlencode($paymentStatusFilter) : ''; ?>" title="First Page">
                                ...
                                <li>
                                    <a href="?page=<?php echo $page - 1; ?><?php echo !empty($search) ? '&search=' . urlencode($search) : '';
                                                                            ...
                                                                            echo !empty($schemeFilter) ? '&scheme_id=' . urlencode($schemeFilter) : '';
                                                                            echo !empty($installmentFilter) ? '&installment_id=' . urlencode($installmentFilter) : '';
                                                                            echo !empty($paymentStatusFilter) ? '&payment_status=' . urlencode($paymentStatusFilter) : ''; ?>" title="Previous Page">
                            ...
                            if ($startPage > 1) {
                                echo '<li><a href="?page=1' . (!empty($search) ? '&search=' . urlencode($search) : '') .
                                    ...
                                    (!empty($referredBy) ? '&referred_by=' . urlencode($referredBy) : '') .
                                    (!empty($schemeFilter) ? '&scheme_id=' . urlencode($schemeFilter) : '') .
                                    (!empty($installmentFilter) ? '&installment_id=' . urlencode($installmentFilter) : '') .
                                    (!empty($paymentStatusFilter) ? '&payment_status=' . urlencode($paymentStatusFilter) : '') . '">1</a></li>';
                                if ($startPage > 2) {
                            ...
                            for ($i = $startPage; $i <= $endPage; $i++) {
                                echo '<li class="' . ($page == $i ? 'active' : '') . '">
                                    ...
                                    (!empty($schemeFilter) ? '&scheme_id=' . urlencode($schemeFilter) : '') .
                                    (!empty($installmentFilter) ? '&installment_id=' . urlencode($installmentFilter) : '') .
                                    (!empty($paymentStatusFilter) ? '&payment_status=' . urlencode($paymentStatusFilter) : '') . '">' . $i . '</a>
                                    </li>';
                            }
                            ...
                            if ($endPage < $totalPages) {
                                ...
                                echo '<li><a href="?page=' . $totalPages .
                                    ...
                                    (!empty($schemeFilter) ? '&scheme_id=' . urlencode($schemeFilter) : '') .
                                    (!empty($installmentFilter) ? '&installment_id=' . urlencode($installmentFilter) : '') .
                                    (!empty($paymentStatusFilter) ? '&payment_status=' . urlencode($paymentStatusFilter) : '') . '">' . $totalPages . '</a></li>';
                            }
                            ?>
                            ...
                            <?php if ($page < $totalPages): ?>
                                <li>
                                    <a href="?page=<?php echo $page + 1; ?><?php echo !empty($search) ? '&search=' . urlencode($search) : '';
                                                                            ...
                                                                            echo !empty($schemeFilter) ? '&scheme_id=' . urlencode($schemeFilter) : '';
                                                                            echo !empty($installmentFilter) ? '&installment_id=' . urlencode($installmentFilter) : '';
                                                                            echo !empty($paymentStatusFilter) ? '&payment_status=' . urlencode($paymentStatusFilter) : ''; ?>" title="Next Page">
                                ...
                                <li>
                                    <a href="?page=<?php echo $totalPages; ?><?php echo !empty($search) ? '&search=' . urlencode($search) : '';
                                                                                ...
                                                                                echo !empty($schemeFilter) ? '&scheme_id=' . urlencode($schemeFilter) : '';
                                                                                echo !empty($installmentFilter) ? '&installment_id=' . urlencode($installmentFilter) : '';
                                                                                echo !empty($paymentStatusFilter) ? '&payment_status=' . urlencode($paymentStatusFilter) : ''; ?>" title="Last Page">
    ...
    <script>
        ...
        $(document).ready(function() {
            // When scheme changes via Select2 UI, dynamically reload installments without page reload
            $('#scheme_id').on('select2:select select2:clear', function() {
                const schemeId = $(this).val();
                const $installment = $('#installment_id');
                $installment.empty();
                $installment.append(new Option('All Installments', '', true, true));
                $installment.trigger('change');

                // If unsubscribed or no scheme, disable and stop
                if (!schemeId || schemeId === 'unsubscribed') {
                    $installment.prop('disabled', true);
                    $installment.trigger('change.select2');
                    return;
                } else {
                ...
                .done(function(items) {
                    if (Array.isArray(items)) {
                        items.forEach(function(item) {
                            const text = item.InstallmentName + ' (' + item.SchemeName + ' #' + item.InstallmentNumber + ')';
                            const option = new Option(text, item.InstallmentID, false, false);
                ...
                .fail(function() {
                    // On error, leave the default only
                });
            });
            ...
            <?php if (!empty($schemeFilter)): ?>
                $('.scheme-select').val('<?php echo $schemeFilter; ?>').trigger('change');
            <?php endif; ?>
            
            <?php if (!empty($installmentFilter)): ?>
                $('.installment-select').val('<?php echo $installmentFilter; ?>').trigger('change');
            <?php endif; ?>
        });
    </script>
</body>
