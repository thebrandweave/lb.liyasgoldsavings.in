...
function sendWhatsAppMessageAsync($phoneNumber, $message, $userId, $userType)
{
    ...
    try {
        ...
        // Use cURL with timeout for faster response
        $ch = curl_init();
...
// Function to queue WhatsApp message for background processing
function queueWhatsAppMessage($phoneNumber, $message, $userId, $userType)
{
    global $conn;
    
    try {
        // Store message in a queue table for background processing
        $stmt = $conn->prepare("
            INSERT INTO WhatsAppQueue (PhoneNumber, Message, UserID, UserType, Status, CreatedAt) 
            VALUES (?, ?, ?, ?, 'Pending', NOW())
        ");
        $stmt->execute([$phoneNumber, $message, $userId, $userType]);
        return true;
    } catch (Exception $e) {
        error_log("Error queuing WhatsApp message: " . $e->getMessage());
        return false;
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (empty($errors)) {
            ...
            // Optimized function to generate unique ID for promoters
            function generatePromoterUniqueID($conn, $maxRetries = 5)
            {
                $attempts = 0;
                ...
                while ($attempts < $maxRetries) {
                    ...
                    // Check if this unique ID already exists (optimized query)
                    $stmt = $conn->prepare("SELECT 1 FROM Promoters WHERE PromoterUniqueID = ? LIMIT 1");
                    $stmt->execute([$uniqueID]);
                    
                    if (!$stmt->fetch()) {
                        return $uniqueID; // Unique ID found
                    }

                    $attempts++;
                }

                // If we reach here, use timestamp-based ID as fallback
                $timestamp = time();
                $uniqueID = 'GDP' . $timestamp;
                return $uniqueID;
            }

            // Generate unique ID (only for promoters, customers use trigger)
            $uniqueID = $registrationType === 'promoter' ? generatePromoterUniqueID($conn) : null;

            if ($registrationType === 'customer') {
                // Insert customer (CustomerUniqueID will be auto-generated by trigger)
