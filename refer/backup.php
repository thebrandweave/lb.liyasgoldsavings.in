            ...
            if ($registrationType === 'promoter' && !empty($referralCode)) {
                ...
                if (is_numeric($commission)) {
                    ...
                    if ($newCommission >= $parentCommission) {
                        $error_message = "Invalid referral link. Commission must be less than parent promoter's commission.";
                    }
                } else {
    ...
    try {
        ...
        if (empty($errors)) {
            ...
            if ($registrationType === 'customer') {
                ...
                $stmt->execute([
                    $promoterData['PromoterID'],
                    "Registered new promoter: $name (ID: $uniqueID) with commission: $commission",
                    $_SERVER['REMOTE_ADDR']
                ]);
            ...
            header("Location: " . ($registrationType === 'customer' ? '../customer/login' : '../promoter/login.php'));
            exit();
        } else {
...
<body>
    ...
    <script>
        function selectRegistrationType(type) {
            window.location.href = 'register.php?id=<?php echo htmlspecialchars($promoterID); ?>&type=' + type<?php echo !empty($referralCode) ? ' + "&ref=' . htmlspecialchars($referralCode) . '"' : ''; ?>;
        }
    </script>
</body>
