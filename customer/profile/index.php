...
// Normalize profile image path so it loads correctly on this page too
$profileImageUrl = '';
if (!empty($customer['ProfileImageURL'])) {
    $stored = $customer['ProfileImageURL'];
    // If the path is relative like "uploads/profile_images/...", prefix editProfile directory
    if (preg_match('~^uploads/|^profile_images/|^uploads\\/~', $stored)) {
        $profileImageUrl = './editProfile/' . ltrim($stored, '/');
    } else {
        $profileImageUrl = $stored; // absolute or already correct
    }
}
...
<head>
    ...
    <title>My Profile - Liyas Gold Savings</title>
